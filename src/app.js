    ...
    anomaliesDiv.innerHTML = anomalies.map(a => `
      <div class="anomaly-item">
        <div>
          ...
          <div style="margin-top: 5px; color: #ffb3ba; font-size: 0.9rem;">
...
function renderCategories() {
  ...
  if (mergingCategoryId) {
    ...
    if (mergingCat) {
      headerHtml = `
        <div style="background: #fff5d6; border: 1px solid #ffd6a5; border-radius: 8px; padding: 15px; margin-bottom: 15px;">
  ...
  const html = categoryStats.map(cat => {
    ...
    return `
      <div class="category-item" style="${isMergingThis ? 'background: #fff5d6;' : ''}">
...
function renderPurposeBudgets() {
  ...
  const html = budgets.map(budget => {
    ...
    const barColor = budget.percentage > 90 ? '#ffb3ba' : (budget.percentage > 75 ? '#ffd6a5' : '#b8f0d8');
